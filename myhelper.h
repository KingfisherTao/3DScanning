...
public:
    ...
    static void formInCenter(QWidget *frm)
    {
        int frmX = frm->width();
        int frmY = frm->height();
        int deskWidth = QGuiApplication::primaryScreen()->geometry().width();
        int deskHeight = QGuiApplication::primaryScreen()->geometry().height();
    ...
    // 输入二进制字符串 返回 十六进制字符串
    static QString BinaryToStrHex(QString str)
    {
        bool ok;
        return decimalToStrHex(str.toInt(&ok, 2));
    }
    ...
    static QString formatString(QString str)
    {
        int n = str.length();
        while(n-2 > 0){
            n = n - 2;
