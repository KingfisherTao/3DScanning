...
SettingsDialog::SettingsDialog(QLabel* lab, QWidget *parent) :
    QDialog(parent),
...
void SettingsDialog::showPortInfo(int idx)
{
    ...
    m_ui->descriptionLabel->setText(QString("描述: %1").arg(list.count() > 1 ? list.at(1) : tr(blankString)));
    m_ui->manufacturerLabel->setText(QString("制造商: %1").arg(list.count() > 2 ? list.at(2) : tr(blankString)));
    m_ui->serialNumberLabel->setText(QString("序列号: %1").arg(list.count() > 3 ? list.at(3) : tr(blankString)));
    m_ui->locationLabel->setText(QString("位置: %1").arg(list.count() > 4 ? list.at(4) : tr(blankString)));
    m_ui->vidLabel->setText(QString("供应商ID: %1").arg(list.count() > 5 ? list.at(5) : tr(blankString)));
    m_ui->pidLabel->setText(QString("产品ID: %1").arg(list.count() > 6 ? list.at(6) : tr(blankString)));
}
...
void SettingsDialog::fillPortsParameters()
{
    ...
    m_ui->parityBox->addItem(QString("无"), QSerialPort::NoParity);
    m_ui->parityBox->addItem(QString("奇"), QSerialPort::OddParity);
    m_ui->parityBox->addItem(QString("偶"), QSerialPort::EvenParity);
#ifdef Q_OS_WIN
    m_ui->parityBox->addItem(QString("标志"), QSerialPort::MarkParity);
#endif
    m_ui->parityBox->addItem(QString("空格"), QSerialPort::SpaceParity);
    ...
    m_ui->flowControlBox->addItem(QString("无"), QSerialPort::NoFlowControl);
    m_ui->flowControlBox->addItem(QString("RTS/CTS"), QSerialPort::HardwareControl);
    m_ui->flowControlBox->addItem(QString("XON/XOFF"), QSerialPort::SoftwareControl);
}
...
void SettingsDialog::updateSettings()
{
    ...
    m_lab->setText(m_currentSettings.name);
}
